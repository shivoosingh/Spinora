...
const POLL_MS = Number(process.env.JUWA_POLL_MS ?? 10_000);
const GAME_SLUG = "juwa";

function createAdminSupabase() {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL ?? process.env.SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) {
    throw new Error("Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in workers/juwa-bot/.env");
  }
...
async function processOne(supabase: ReturnType<typeof createAdminSupabase>) {
  ...
  console.log(`[juwa-bot] Processing job ${job.id} — $${job.amount} (${job.load_type})`);

  const enrichedJob = await enrichGameLoadJob(supabase, job as GameLoadJob);

  try {
    const result = await runJuwaJob(enrichedJob, supabase);
    ...
    const isCreate = enrichedJob.load_type === "create_account" || enrichedJob.load_type === "new_account";
    ...
    if (notifyError) console.warn("[juwa-bot] Notification failed:", notifyError.message);
    console.log(`[juwa-bot] Completed ${job.id} → user ${result.username}`);
  } catch (err) {
...
async function main() {
  ...
  console.log("[juwa-bot] Started — polling for Juwa wallet load jobs");
  ...
  do {
    try {
      ...
      console.error("[juwa-bot] Poll error (continuing):", err);
      if (once) break;
      await new Promise((r) => setTimeout(r, POLL_MS));
    }
  } while (!once);

  console.log("[juwa-bot] Done");
}
