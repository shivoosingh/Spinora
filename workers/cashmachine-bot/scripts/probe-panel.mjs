...
const url = process.env.CASHMACHINE_ADMIN_URL ?? "https://agentserver.cashmachine777.com/admin/login";
const base = url.replace(/\/login.*$/i, "");
const cdpUrl = process.env.CASHMACHINE_CDP_URL?.trim() || "http://127.0.0.1:9226";
...
const page =
  pages.find((p) => p.url().includes("cashmachine777.com")) ?? pages[0] ?? (await context.newPage());
