...
const GAME_SLUG = "cash-frenzy";

function createAdminSupabase() {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL ?? process.env.SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) {
    throw new Error("Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in workers/cash-frenzy-bot/.env");
  }
...
async function processOne(supabase: ReturnType<typeof createAdminSupabase>) {
  ...
  try {
    ...
    if (notifyError) console.warn("[cash-frenzy-bot] Notification failed:", notifyError.message);
    console.log(`[cash-frenzy-bot] Completed ${job.id} → user ${result.username}`);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[cash-frenzy-bot] Failed ${job.id}:`, message);
    try {
      await complete(supabase, job.id, false, { error: message.slice(0, 500) });
    } catch (completeErr) {
      console.error("[cash-frenzy-bot] Could not mark job failed:", completeErr);
    }
  }
...
async function main() {
  ...
  console.log("[cash-frenzy-bot] Done");
}
