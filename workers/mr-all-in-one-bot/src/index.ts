...
const POLL_MS = Number(process.env.MRALLINONE_POLL_MS ?? 10_000);
const GAME_SLUG = "mr-all-in-one";

function createAdminSupabase() {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL ?? process.env.SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) {
    throw new Error("Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in workers/mr-all-in-one-bot/.env");
  }
...
async function processOne(supabase: ReturnType<typeof createAdminSupabase>) {
  ...
  console.log(`[mr-all-in-one-bot] Processing job ${job.id} — $${job.amount} (${job.load_type})`);
  ...
  try {
    ...
    if (notifyError) console.warn("[mr-all-in-one-bot] Notification failed:", notifyError.message);
    console.log(`[mr-all-in-one-bot] Completed ${job.id} → user ${result.username}`);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[mr-all-in-one-bot] Failed ${job.id}:`, message);
    try {
      await complete(supabase, job.id, false, { error: message.slice(0, 500) });
    } catch (completeErr) {
      console.error("[mr-all-in-one-bot] Could not mark job failed:", completeErr);
    }
  }
...
async function main() {
  ...
  console.log("[mr-all-in-one-bot] Started — polling for MR All In One wallet load jobs");
  ...
  do {
    try {
      ...
      console.error("[mr-all-in-one-bot] Poll error (continuing):", err);
      if (once) break;
      await new Promise((r) => setTimeout(r, POLL_MS));
    }
  } while (!once);

  console.log("[mr-all-in-one-bot] Done");
}
