...
const cdpUrl = process.env.GAMEROOM_CDP_URL?.trim() || "http://127.0.0.1:9224";
const base = (process.env.GAMEROOM_ADMIN_URL ?? "https://agentserver1.gameroom777.com/login").replace(
  /\/login.*$/i,
  ""
);
...
const page =
  pages.find((p) => p.url().includes("gameroom777.com")) ?? pages[0] ?? (await context.newPage());
...
try {
  ...
  const account = process.env.PROBE_ACCOUNT || "anthony7999";
